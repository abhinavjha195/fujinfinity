...
function timeline(e, v) {
    var g = [],
        ...
        autoSlideTimer = null, // Timer variable for auto slide
        ...
        o = e.settings.rtlMode ? e.items.length > e.settings.visibleItems ? e.items.length - e.settings.visibleItems : 0 : e.settings.startIndex, e.timelineEl.classList.add("timeline--horizontal"),
            s = e, window.innerWidth > s.settings.forceVerticalMode && (s.itemWidth = s.wrap.offsetWidth / s.settings.visibleItems, s.items.forEach(function (e) {
                ...
                function () {
                    ...
                    s.items.forEach(function (e, t) {
                        t % 2 == 0 ? (e.style.height = l + "px", "bottom" === s.settings.horizontalStartPosition ? (e.classList.add("timeline__item--bottom"), d(e, i)) : e.classList.add("timeline__item--top")) :
                    ...
                    if (e.items.length > e.settings.visibleItems) {
                        ...
                        t.className = "timeline-nav-button timeline-nav-button--prev", i.className = "timeline-nav-button timeline-nav-button--next", t.textContent = "Previous", i.textContent = "Next",
                            t.style.top = n + "px", i.style.top = n + "px", 0 === o ? t.disabled = !0 : o === e.items.length - e.settings.visibleItems && (i.disabled = !0), e.timelineEl.appendChild(t),
                ...
                i = (t = e).timelineEl.querySelectorAll(".timeline-nav-button"), n = t.timelineEl.querySelector(".timeline-nav-button--prev"), l = t.timelineEl.querySelector(".timeline-nav-button--next"), a = t.items.length - t.settings.visibleItems, r = parseInt(t.settings.moveItems, 10), [].forEach.call(i, function (e) {
    ...
    function startAutoSlide(e) {
        autoSlideTimer = setInterval(function () {
            var nextButton = e.timelineEl.querySelector(".timeline-nav-button--next");
            var prevButton = e.timelineEl.querySelector(".timeline-nav-button--prev");
            
            if (!nextButton.disabled) {
                // If next button is enabled, click on it
                nextButton.click();
            } else {
                // If next button is disabled (reached end), simulate a click on the first item
                prevButton.click();
                o = 0; // Reset the current index to the first item
                c(e); // Reset the position of the scroller
            }
        }, autoSlideInterval);
    }
    ...
    function a() {
        g.forEach(function (e) {
            e.timelineEl.style.opacity = 0, e.timelineEl.classList.contains("timeline--loaded") || e.items.forEach(function (e) {
                n(e.querySelector(".timeline__content"), document.createElement("div"), "timeline__content__wrap"),
                    n(e.querySelector(".timeline__content__wrap"), document.createElement("div"), "timeline__item__inner")
            }),
                function (e) {
                ...
                "horizontal" === e.settings.mode && window.innerWidth > e.settings.forceVerticalMode ? s(e) : function (i) {
                    ...
                    i.items.forEach(function (e, t) {
                        e.classList.remove("animated", "fadeIn"), !l(e, i.settings.verticalTrigger) && 0 < t ? e.classList.add("animated") : n = t,
                ...
                }(e), e.timelineEl.classList.add("timeline--loaded"),
                setTimeout(function () {
    ...
    e.length && [].forEach.call(e, function (e) {
        ...
        Object.keys(b).forEach(function (e) {
            var t, i;
            r[e] = b[e].defaultValue, n[e] ? r[e] = n[e] : v && v[e] && (r[e] = v[e]),
                "integer" === b[e].type ? r[e] && (t = r[e], i = e, "number" == typeof t || t % 1 == 0 || (console.warn(p + ' The value "' + t + '" entered for the setting "' + i + '" is not an integer.'), 0)) || (r[e] = b[e].defaultValue) :
        ...
        r.verticalTrigger = {
            ...
            r.startIndex > a.length - r.visibleItems && a.length > r.visibleItems ? (console.warn(p + " The 'startIndex' setting must be between 0 and " + (a.length - r.visibleItems) + " for this timeline. The value of " + (a.length - r.visibleItems) + " has been used instead."), r.startIndex = a.length - r.visibleItems) :
            a.length <= r.visibleItems ? (console.warn(p + " The number of items in the timeline must exceed the number of visible items to use the 'startIndex' option."), r.startIndex = 0) :
    ...
    // Start auto slide when the timeline is loaded
    startAutoSlide(g[0]);
    // Reset auto slide timer on every click
    g.forEach(function (timelineInstance) {
        timelineInstance.timelineEl.addEventListener("click", function () {
            stopAutoSlide();
            startAutoSlide(timelineInstance);
        });
    });
